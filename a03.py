...
points1 = sum([points(r[0].intersection(r[1]).pop()) for r in rucksacks])
print("Part 1:", points1)

chunks = [lines[i:i+3] for i in range(0,len(lines),3)]
points2 = sum([points(set(c[0]).intersection(set(c[1])).intersection(set(c[2])).pop()) for c in chunks])
